...
async function handleGetTemplates(event): Promise<void> {
  try {
    const {data}: {data: TemplateData} = await axios.get(`${process.env.FASTIFY_URL}/templates`);
...
async function handleGetDetailTemplate(event, templateId: string): Promise<void> {
  try {
