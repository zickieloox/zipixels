...
async function handleUploadToServer(event, fileName: string, fileSize: number): Promise<void> {
  try {
...
const processChildren = async (children, uploadedFiles = [], uploadedImages) => {
  const folderPath = './';
  const currentTime = new Date().toLocaleDateString('en-ZA');
